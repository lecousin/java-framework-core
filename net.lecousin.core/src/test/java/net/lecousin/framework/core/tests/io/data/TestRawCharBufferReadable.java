...
public class TestRawCharBufferReadable extends TestCharsReadable {

	public TestRawCharBufferReadable(char[] data, int initialPos, int length, boolean useSubBuffer) {
		super(initialPos == 0 && length == data.length ? new RawCharBuffer(data) : new RawCharBuffer(data, initialPos, length), data, initialPos, length, useSubBuffer);
	}
	...
	public void testDuplicate() {
		...
		RawCharBuffer d = ((RawCharBuffer)buffer).duplicate();
	...
	public void testFromCharBuffer() {
		...
		RawCharBuffer rb = new RawCharBuffer(b);
