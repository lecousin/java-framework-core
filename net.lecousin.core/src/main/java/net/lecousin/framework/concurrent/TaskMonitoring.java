...
public final class TaskMonitoring {
	...
	public static int monitoringInterval = 60 * 1000;
	public static int secondsBeforeToPutTaskAside = 5 * 60;
	public static int secondsBeforeToKillTask = 10 * 60;
	...
	private static class TaskMonitor implements Runnable, Closeable {
		...
		public void run() {
			while (!closed) {
				synchronized (lock) {
					try { lock.wait(monitoringInterval); }
		...
		private static void check(TaskWorker worker) {
			...
			if (seconds < secondsBeforeToPutTaskAside) return;
			if (seconds < secondsBeforeToKillTask) {
