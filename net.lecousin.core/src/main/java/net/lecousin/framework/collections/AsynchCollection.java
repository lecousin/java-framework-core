...
public interface AsynchCollection<T> {
	...
	/**
	 * Aggregates several AsynchCollection into a single one:
	...
	public static class Aggregator<T> implements AsynchCollection<T> {
		...
		/** Constructor. */
		public Aggregator(int nbSubCollection, AsynchCollection<T> mainCollection) {
	...
	/**
	 * AsynchCollection but where elements are expected to come only one by one instead of by bunch.
	 * @param <T> type of elements
	 */
	...
	public static class Keep<T> implements AsynchCollection<T> {
		...
		@Override
		public void newElements(Collection<T> elements) {
			ArrayList<AsynchCollection<T>> toCall;
			synchronized (this) {
				list.addAll(elements);
				toCall = new ArrayList<>(listeners);
			}
			for (AsynchCollection<T> col : toCall)
		...
		public void done() {
			...
			for (AsynchCollection<T> col : listeners)
				col.done();
			listeners = null;
		 ...
		 */
		public void provideTo(AsynchCollection<T> col) {
