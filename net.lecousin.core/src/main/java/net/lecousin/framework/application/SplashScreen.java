...
public class SplashScreen extends Thread implements WorkProgress {
	...
	public void run() {
		synchronized (this) {
			...
			progressText.setFont(new Font("Arial", 0, 10));
			...
			progressSubText.setFont(new Font("Arial", 0, 10));
		...
		if (devMode) {
			...
			for (int i = 0; i < 20; ++i) {
				...
				if (win == null) break;
				if (LCCore.get() != null && LCCore.get().isStopping()) break;
			}
			close();
		}
	}
	...
	private void update() {
		...
		if (logo != null && isCustomLogo) {
			if (bottom.getComponentCount() == 2) {
		...
		int width = 400;
		int height = 0;
		if (logoContainer.getComponentCount() > 0)
